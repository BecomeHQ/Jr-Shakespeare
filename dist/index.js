(() => {
  ...
  window.addEventListener("DOMContentLoaded", (event) => {
    ...
    const heroInteraction = () => {
      ...
      heroTl.from(".navbar_component", { y: "-100%", duration: 1 }).from(".section_home-hero", { opacity: 0, duration: 0.5 }, "-=0.8").from(".hero_title .char", {
        // y: '-20px',
        opacity: 0,
        duration: 1,
        stagger: { amount: 1.2 },
        ease: "power1.out"
      }).from(".hero_content", { y: "-20px", opacity: 0, duration: 0.5 }, "-=0.5").from(".hero_content_line path", { strokeDashoffset: 170, duration: 0.5 }).from(
        ".up-arrow",
        {
          y: 10,
          opacity: 0,
    ...
    const bookSequenceInteraction = () => {
      const bookSequenceTl = gsapWithCSS.timeline({
        scrollTrigger: {
          trigger: ".section_books",
          start: "top 75%"
        }
      });
      bookSequenceTl.from(".book_sequence_col-1 .book-item-wrapper", {
        opacity: 0,
        y: -50,
        stagger: { amount: 1.5 },
        duration: 0.5,
    ...
    const randomContentInteraction = () => {
      ...
      randomContentTl.from(".random_content_item", {
        y: "40px",
        opacity: 0,
        stagger: 0.4
      }).fromTo(
